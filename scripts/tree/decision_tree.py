...
def parse_args():
    ...
    # comment this out when using dino bc i just hardcoded the path for simplicity
...
def load_flat_data_resnet(feature_dir):
    # resnet features
...
def load_flat_data_dino():
    # dinov2 features
    X = np.load("/data_full/dino_per_frame_concat/X.npy", mmap_mode="r")   # (total_frames, 2048)
    y = np.load("/data_full/dino_per_frame_concat/y.npy", mmap_mode="r")   # (total_frames,)
...
def main():
    ...
    X_flat, y_flat = load_flat_data_dino()
